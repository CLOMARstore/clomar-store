...
def get_engine(url: str):
    """Motor de base optimizado para Streamlit Cloud + Neon."""
...
def sql_id():
    # En PostgreSQL se usa IDENTITY. Antes se usaba BIGSERIAL y podía dejar
    # secuencias huérfanas si una creación de tabla fallaba a medias.
    return "BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY" if IS_POSTGRES else "INTEGER PRIMARY KEY AUTOINCREMENT"
...
def exec_sql(sql: str, params: dict | None = None):
    ...
    # Cada escritura/migración limpia caché para que ventas, stock y reportes
    # se actualicen sin reiniciar la app.
    try:
        st.cache_data.clear()
    except Exception:
        pass
...
def _params_tuple(params: dict | None):
    params = params or {}
    return tuple(sorted((str(k), str(v)) for k, v in params.items()))


@st.cache_data(ttl=20, show_spinner=False)
def _query_df_cached(url: str, sql: str, params_tuple: tuple) -> pd.DataFrame:
...
def query_df(sql: str, params: dict | None = None) -> pd.DataFrame:
    # Caché corto: acelera navegación y reportes. Se limpia automáticamente
    # cuando se registra una venta, producto, compra, caja, usuario, etc.
    return _query_df_cached(database_url(), sql, _params_tuple(params))
...
# ESQUEMA CLOUD
# ============================================================
def cleanup_orphan_sequences():
    """Elimina secuencias huérfanas que pueden quedar tras un deploy fallido."""
    if not IS_POSTGRES:
        return
    pairs = [
        ("usuarios", "usuarios_id_usuario_seq"),
        ("categorias", "categorias_id_categoria_seq"),
        ("clientes", "clientes_id_cliente_seq"),
        ("proveedores", "proveedores_id_proveedor_seq"),
        ("productos", "productos_id_producto_seq"),
        ("ventas", "ventas_id_venta_seq"),
        ("detalle_ventas", "detalle_ventas_id_detalle_seq"),
        ("movimientos_stock", "movimientos_stock_id_movimiento_seq"),
        ("compras", "compras_id_compra_seq"),
        ("detalle_compras", "detalle_compras_id_detalle_compra_seq"),
        ("caja", "caja_id_caja_seq"),
        ("cierres_caja", "cierres_caja_id_cierre_seq"),
        ("egresos", "egresos_id_egreso_seq"),
    ]
        ...
        try:
            ...
            BEGIN
                IF to_regclass('public.{table}') IS NULL THEN
                    DROP SEQUENCE IF EXISTS public.{seq};
                END IF;
            END $$;
            """)
        except Exception:
...
@st.cache_resource(show_spinner=False)
def init_db_once(url: str):
    # Evita ejecutar DDL/migraciones en cada clic o cambio de pantalla.
    init_db()
    return True


try:
    init_db_once(database_url())
    DB_OK = True
    DB_ERROR = ""
...
/* Corrección visual V24.2: textos siempre legibles */
html, body, .stApp, p, span, div { font-family: Inter, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; }
label, [data-testid="stWidgetLabel"], [data-testid="stWidgetLabel"] p,
.stTextInput label, .stTextArea label, .stSelectbox label, .stNumberInput label, .stDateInput label, .stRadio label {
  color:#111827 !important; font-weight:800 !important; opacity:1 !important;
}
